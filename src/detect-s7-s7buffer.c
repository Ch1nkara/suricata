...
#include "detect-s7-s7buffer.h"
#include "app-layer-parser.h"
#include "detect-engine-build.h"
#include "rust.h"

static int DetectS7S7BufferSetup(DetectEngineCtx *, Signature *, const char *);
static uint8_t DetectEngineInspectS7S7Buffer(DetectEngineCtx *de_ctx,
        DetectEngineThreadCtx *det_ctx, const struct DetectEngineAppInspectionEngine_ *engine,
        const Signature *s, Flow *f, uint8_t flags, void *alstate, void *txv, uint64_t tx_id);
#ifdef UNITTESTS
static void DetectS7S7BufferRegisterTests(void);
#endif
static int g_s7_rust_id = 0;

void DetectS7S7BufferRegister(void)
{
    sigmatch_table[DETECT_AL_S7_BUFFER].name = "s7_s7buffer";
    sigmatch_table[DETECT_AL_S7_BUFFER].desc =
            "S7 content modifier to match on the s7 buffers";
    sigmatch_table[DETECT_AL_S7_BUFFER].Setup = DetectS7S7BufferSetup;
#ifdef UNITTESTS
    sigmatch_table[DETECT_AL_S7_BUFFER].RegisterTests = DetectS7S7BufferRegisterTests;
#endif
    ...
    DetectAppLayerInspectEngineRegister2("s7_buffer", ALPROTO_S7, SIG_FLAG_TOSERVER, 0,
            DetectEngineInspectS7S7Buffer, NULL);
...
static int DetectS7S7BufferSetup(DetectEngineCtx *de_ctx, Signature *s, const char *str)
{
...
static int DetectS7S7BufferTest(void)
{
...
static void DetectS7S7BufferRegisterTests(void)
{
    UtRegisterTest("DetectS7S7BufferTest",
        DetectS7S7BufferTest);
}
#endif /* UNITTESTS */
