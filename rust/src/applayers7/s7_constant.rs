...
impl S7Rosctr {
    pub fn from_u8(input_u8: u8) -> Result<Self, String> {
        match input_u8 {
            ...
            _ => Err(format!("'{}' cannot be converted with 
                    S7Rosctr::from_u8", input_u8)),
        }
    }
}
...
impl std::str::FromStr for S7Item {
    ...
    fn from_str(input_string: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 4 {
            return Err(format!("Error parsing '{}' with S7Item::from_str: 
                    wrong length", input_string));
        }
        let db_number = match parts.pop().unwrap_or("EOF").parse() {
            Ok(result) => result,
            _ => return Err(format!("Error parsing '{}' with S7Item::from_str: 
                    first element", input_string))
        };
        let transport_size = match S7TransportSize::from_str(parts.pop().unwrap_or("EOF")) {
            Ok(result) => result,
            _ => return Err(format!("Error parsing '{}' with S7Item::from_str: 
                    second element", input_string))
        };

        let address: Vec<&str> = parts.pop().unwrap_or("EOF").split('.').collect();
        if address.len() != 2 {
            return Err(format!("Error parsing '{}' with S7Item::from_str: 
            third element", input_string));
        }
...
impl std::str::FromStr for S7TransportSize {
    ...
    fn from_str(input_string: &str) -> Result<Self, Self::Err> {
        match input_string {
            ...
            _ => Err(format!("'{}' cannot be converted with 
                    S7TransportSize::from_str", input_string)),
        }
    }
}
...
impl std::str::FromStr for S7SignatureType {
    ...
    fn from_str(input_string: &str) -> Result<Self, Self::Err> {
        match input_string {
            ...
            _ => Err(format!("'{}' cannot be converted with 
                    S7SignatureType::from_str", input_string)),
        }
    }
}
